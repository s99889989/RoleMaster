...
public class RoleHoloModelMechanic implements ITargetedEntitySkill, ITargetedLocationSkill {
    ...
    //加上玩家仰角
    private boolean caster_pitch;
    //加上玩家偏航角
    private boolean caster_yaw;

    public RoleHoloModelMechanic(CustomMechanic customMechanic, MythicLineConfig mythicLineConfig) {


        this.mark = mythicLineConfig.getString(new String[]{"mark"}, "");
        this.model_id = mythicLineConfig.getString(new String[]{"model", "m"}, "");
        this.animation = mythicLineConfig.getString(new String[]{"animation", "a"}, "");
        ...
        this.caster_pitch = mythicLineConfig.getBoolean(new String[]{"caster_pitch", "cp"}, false);
        this.caster_yaw = mythicLineConfig.getBoolean(new String[]{"caster_yaw", "cy"}, false);
    ...
    public void setHolo(Entity casterEntity, Location inpuLocation){
        ...
        if(roleHoloMap.containsKey(mark)){
            ...
            roleHolo.setCaster_pitch(this.caster_pitch);
            roleHolo.setCaster_yaw(this.caster_yaw);
            if(this.teleport){
                roleHolo.setLocation(inpuLocation, 0, y, 0);
            }
            roleHolo.setAnimation(animation);
            ...
            RoleHolo roleHolo = new RoleHolo(mark, casterEntity, inpuLocation, 0, y, 0, this.caster_pitch, this.caster_yaw);

            roleHolo.setModel(this.model_id);
            roleHolo.setDuration(duration);
